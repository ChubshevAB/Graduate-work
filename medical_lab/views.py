...
class PatientViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ["list", "retrieve"]:
            ...
            permission_classes = [
                IsModerator
            ]  # Только модераторы могут создавать пациентов
    ...
    @action(
        detail=False, methods=["get"], permission_classes=[IsAdministratorOrModerator]
    )
...
class AnalysisViewSet(viewsets.ModelViewSet):
    ...
    def dashboard_stats(self, request):
        ...
        stats = {
            ...
            "completion_rate": (
                round((completed_analyses / total_analyses * 100), 2)
                if total_analyses > 0
                else 0
            ),
        }
...
def patients_list(request):
    ...
    if not request.user.is_authenticated:
        messages.error(
            request, "Для доступа к этой странице необходимо авторизоваться."
        )
...
def create_patient(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if request.user.is_moderator:
                ...
                patient.save()
                messages.success(
                    request, f"Пациент {patient.get_full_name()} успешно создан!"
                )
    ...
    context = {"form": form, "page_title": "Добавить пациента"}
...
def edit_patient(request, patient_id):
    ...
    if not request.user.is_authenticated:
        messages.error(
            request, "Для редактирования пациента необходимо авторизоваться."
        )
        return redirect("users:login")
    ...
    if not (
        ...
        or (request.user.is_regular_user and patient.created_by == request.user)
    ):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, f"Данные пациента {patient.get_full_name()} успешно обновлены!"
            )
...
def create_analysis(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            analysis = form.save()
            messages.success(
                request, f"Анализ {analysis.analysis_type.name} успешно создан!"
            )
            return redirect("medical_lab:analyses_list")
    else:
        form = AnalysisForm(user=request.user, initial=initial_data)

    context = {"form": form, "page_title": "Новый анализ"}
...
def edit_analysis(request, analysis_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, f"Анализ {analysis.analysis_type.name} успешно обновлен!"
            )
...
def add_analysis_result(request, analysis_id):
    ...
    if not request.user.is_moderator:
        messages.error(
            request, "Недостаточно прав для добавления результатов анализов."
        )
        return redirect("medical_lab:analyses_list")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Результат анализа {analysis.analysis_type.name} успешно добавлен!",
            )
            return redirect("medical_lab:analysis_detail", analysis_id=analysis.id)
    else:
...
def reports(request):
    ...
    analysis_types_stats = (
        analyses_queryset.values("analysis_type__name")
