...
class AnalysisForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['analysis_type'].queryset = AnalysisType.objects.filter(is_active=True)

        # Настройка виджетов
        self.fields['patient'].widget.attrs.update({'class': 'form-select'})
        self.fields['analysis_type'].widget.attrs.update({'class': 'form-select'})
        self.fields['collection_date'].widget = forms.DateTimeInput(attrs={
            'class': 'form-control',
            'type': 'datetime-local'
        })
...
class AnalysisTypeForm(forms.ModelForm):
    ...
    class Meta:
        model = AnalysisType
        fields = ['name', 'description', 'price', 'preparation_instructions', 'turnaround_time', 'is_active']
        widgets = {
            'name': forms.TextInput(attrs={'class': 'form-control'}),
            'description': forms.Textarea(attrs={'class': 'form-control', 'rows': 3}),
            'price': forms.NumberInput(attrs={'class': 'form-control'}),
            'preparation_instructions': forms.Textarea(attrs={'class': 'form-control', 'rows': 4}),
