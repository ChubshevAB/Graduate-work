...
class Patient(models.Model):
    ...
    medical_history = models.TextField(
        verbose_name="Медицинская история", blank=True, null=True
    )
...
class AnalysisType(models.Model):
    ...
    price = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Стоимость", default=0.00
    )
    preparation_instructions = models.TextField(
        verbose_name="Инструкции по подготовке", blank=True
    )
    turnaround_time = models.IntegerField(
        verbose_name="Срок выполнения (в днях)", default=1
    )
...
class Analysis(models.Model):
    ...
    # Результаты
    result = models.TextField(verbose_name="Результат анализа", blank=True)
    ...
    normal_range = models.TextField(verbose_name="Нормальные значения", blank=True)
    ...
    def send_completion_email(self):
        ...
        try:
            ...
            logger.info(
                f"Email отправлен пациенту {self.patient.email} для анализа {self.id}"
            )
...
class Report(models.Model):
    ...
    generated_by = models.ForeignKey(
        User, on_delete=models.CASCADE, verbose_name="Создатель отчета"
    )

    generated_at = models.DateTimeField(auto_now_add=True, verbose_name="Дата создания")

    date_from = models.DateField(verbose_name="Период с", null=True, blank=True)

    date_to = models.DateField(verbose_name="Период по", null=True, blank=True)

    data = models.JSONField(verbose_name="Данные отчета", blank=True, null=True)

    file = models.FileField(
        upload_to="reports/", verbose_name="Файл отчета", null=True, blank=True
    )
